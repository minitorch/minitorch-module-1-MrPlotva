...
def test_sigmoid(a: float) -> None:
    ...
    if b != a:
        assert sigmoid(a) <= sigmoid(
            b
        ), f"sigmoid is not increasing: sigmoid({a}) > sigmoid({b})"
...
def test_transitive(a: float, b: float, c: float) -> None:
    ...
    if lt(a, b) and lt(b, c):
        assert lt(a, c), f"error in transitive property (a={a}, b={b}, c={c})"
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    ls1_trunc = ls1[:min_len]
    ls2_trunc = ls2[:min_len]

    left = sum(ls1_trunc) + sum(ls2_trunc)

    right = sum(addLists(ls1_trunc, ls2_trunc))
